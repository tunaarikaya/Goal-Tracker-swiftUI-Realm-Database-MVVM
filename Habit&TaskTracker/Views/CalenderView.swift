                            ...
                            Text("00")
                                ...
                                .overlay(
                                    Text(weekDayformatter.string(from: date))
                                        .font(.system(size: 15))
                                        .foregroundColor(
                                            calendar.isDate(date, inSameDayAs: selectDate) ? Color.white : calendar.isDateInToday(date) ? Color("color")
                                            : .black
                                        )
                                )
                            Text("00")
                                ...
                                .overlay(
                                    Circle()
                                        .foregroundColor(
                                            calendar.isDate(date, inSameDayAs: selectDate) ? Color.white : calendar.isDateInToday(date) ? Color("color").opacity(0.1)
                                            : .white.opacity(0.5)
                                        )
                                .overlay(
                                    ...
                                    Text(dayFormatter.string(from: date))
                                        .foregroundColor(
                                            calendar.isDate(date, inSameDayAs: selectDate) ? Color.black : calendar.isDateInToday(date) ? Color("color")
                                            : .black
                                        )
                                )
